...
<template>
    <main class="
        ...
        <section class="
            ...
            <!-- SELECT BUTTON -->
            <SelectButton v-model="studentType" :options="studentOptions" optionLabel="label"
                class="w-full text-lg md:col-span-2" :pt="{
                    root: 'flex w-full',
            ...
            <form @submit="signup" class="grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-4">
                ...
                <div class="flex flex-col gap-2 md:col-span-2">
                    ...
                    <InputText v-model="user.nome" placeholder="Digite seu nome" class="w-full text-lg h-[48px]"
                        :pt="{ root: 'h-[48px]' }" />
                </div>
                <!-- INSTITUIÇÃO -->
                <div class="flex flex-col gap-2">
                    <label class="text-xl dark:text-gray-200">Instituição</label>
                    <Select v-model="institution" :options="institutions" optionLabel="label" placeholder="UECE"
                        disabled class="w-full h-[48px] opacity-70 cursor-not-allowed" :pt="{
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <Select v-model="user.idCurso" :options="courses" optionLabel="nome" placeholder="Selecione o curso"
                        class="w-full h-[48px]" :pt="{
                            root: 'w-full h-[48px] rounded-lg border dark:bg-zinc-700 flex items-center',
                            input: 'px-3 text-lg'
                        }" />
                </div>
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <InputText v-model="user.cpf" placeholder="Digite seu CPF" class="w-full text-lg h-[48px]"
                        :pt="{ root: 'h-[48px]' }" />
                </div>
                <!-- EMAIL -->
                <div class="flex flex-col gap-2">
                    <label class="text-xl dark:text-gray-200">E-mail</label>
                    <InputText v-model="user.email" placeholder="Digite seu email" class="w-full text-lg h-[48px]"
                        :pt="{ root: 'h-[48px]' }" />
                </div>
                <!-- SENHA -->
                <div class="flex flex-col gap-2">
                    <label class="text-xl dark:text-gray-200">Senha</label>
                    <Password v-model="user.senha" placeholder="Digite sua senha" :feedback="false" toggleMask
                        inputClass="w-full h-[48px]" class="text-lg"
                        :pt="{ root: 'w-full', input: 'h-[48px] px-3 text-lg' }" />
                </div>
                <!-- CONFIRMAR SENHA -->
                <div class="flex flex-col gap-2 mb-5">
                    <label class="text-xl dark:text-gray-200">Confirmar senha</label>
                    <Password v-model="passwordConfirm" placeholder="Confirme sua senha" :feedback="false" toggleMask
                        inputClass="w-full h-[48px]" class="text-lg"
                        :pt="{ root: 'w-full', input: 'h-[48px] px-3 text-lg' }" />
                </div>
                <!-- CONTINUAR (Criar a lógica de envio ao back)-->
                ...
                <!-- ENTRAR -->
                <Button label="entrar" class="w-full md:col-span-2 -mt-2" @click="$router.push('/login')" />
            </form>
        </section>
    </main>
</template>